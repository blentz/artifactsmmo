...
class BuyBankExpansionAction(Action):
    ...
    def apply(self, state: WorldState, game_data: GameData) -> WorldState:
        ...
        # capacity (None), seed from game_data.bank_capacity — the cycle's
        # snapshot — so the projection starts from the real server value.
        pre_cap = state.bank_capacity if state.bank_capacity is not None else game_data.bank_capacity
