...
class GameData:
    ...
    def _hydrate_bundle(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            # Absent in bundles written before CACHE_VERSION 5 (and in the
            # committed scenario fixture). Missing means "no achievement is
            # known to be completed", which makes every `achievement_unlocked`
            # access condition evaluate UNMET — conditional tiles are excluded
            # rather than assumed reachable. That is the fail-closed direction
            # this feature requires; a default-pass would let the planner route
            # through gated tiles it cannot actually enter.
            "achievements": [AccountAchievementSchema.from_dict(d)
