...
class MoveToWorkshopAction(ActionBase):
    ...
    def __init__(self, char_name: str, target_x: int, target_y: int):
        ...
        Args:
            ...
            target_x: Target X coordinate for workshop location
            target_y: Target Y coordinate for workshop location
        """
        super().__init__()
    ...
    def execute(self, client, **kwargs) -> Optional[Dict]:
        """ Move character to the workshop location """
        ...
        try:
            ...
            error_response = self.get_error_response(f"Move to workshop failed: {str(e)}")
            self.log_execution_result(error_response)
            return error_response

    def __repr__(self):
        return f"MoveToWorkshopAction({self.char_name}, {self.target_x}, {self.target_y})"