...
class FindXpSourcesAction(ActionBase):
    ...
    def __init__(self, skill: str, **kwargs):
        ...
        Args:
            skill: Target skill to find XP sources for (e.g., 'weaponcrafting', 'mining')
            **kwargs: Additional parameters including knowledge_base and learning_manager
        """
        super().__init__()
        self.skill = skill
        self.kwargs = kwargs
    ...
    def execute(self, client, **kwargs) -> Optional[Dict]:
        ...
        self.log_execution_start(skill=self.skill)
        
        try:
            # Get learning manager and knowledge base from context
            learning_manager = kwargs.get('learning_manager') or self.kwargs.get('learning_manager')
            knowledge_base = kwargs.get('knowledge_base') or self.kwargs.get('knowledge_base')
            ...
            if not effects_available:
                self.logger.info(f"🔍 Effects data not available, learning all effects...")
            ...
            xp_sources = learning_manager.find_xp_sources_for_skill(self.skill)
            
            if not xp_sources:
                # Analyze if we should look for alternative skill names
                alternative_skills = self._find_alternative_skill_names(self.skill)
            ...
            if not xp_sources:
                return self.get_error_response(f"No XP sources found for skill '{self.skill}'")
            
            # Analyze XP sources to find actionable items
            actionable_sources = self._analyze_actionable_sources(xp_sources, knowledge_base)
            ...
            result = self.get_success_response(
                ...
                message=f"Found {len(xp_sources)} XP sources for {self.skill} skill"
            )
    ...
    def _find_alternative_skill_names(self, skill: str) -> List[str]:
        """Find alternative skill names by analyzing patterns in the knowledge base."""
        alternatives = []
        
        try:
            # Get knowledge base and learning manager from context
    ...
    def _analyze_actionable_sources(self, xp_sources: List[Dict], knowledge_base) -> List[Dict]:
        """
        ...
        try:
            for source in xp_sources:
                ...
                if 'craft' in effect_name or 'craft' in effect_description:
                    ...
                    self._find_craftable_items_for_skill(actionable_info, knowledge_base)
                ...
                if any(keyword in effect_name for keyword in ['gather', 'harvest', 'mine', 'chop', 'fish']):
                    ...
                    self._find_gatherable_resources_for_skill(actionable_info, knowledge_base)
                ...
                if any(keyword in effect_name for keyword in ['consume', 'drink', 'eat', 'potion']):
                    ...
                    self._find_consumable_items_for_skill(actionable_info, knowledge_base)
    ...
    def _find_craftable_items_for_skill(self, actionable_info: Dict, knowledge_base) -> None:
        ...
        try:
            ...
            for item_code, item_data in items.items():
                ...
                if craft_data:
                    ...
                    if craft_skill and craft_skill.lower() == self.skill.lower():
    ...
    def _find_gatherable_resources_for_skill(self, actionable_info: Dict, knowledge_base) -> None:
            ...
            for resource_code, resource_data in resources.items():
                ...
                for drop in drops:
                    if isinstance(drop, dict):
                        ...
                        if self._is_item_used_for_skill(drop_code, knowledge_base):
                            actionable_info['required_items'].append({
                                'item_code': resource_code,
                                'item_name': resource_data.get('name', resource_code),
                                'action_type': 'gather',
    ...
    def _is_item_used_for_skill(self, item_code: str, knowledge_base) -> bool:
        ...
        try:
            ...
            for check_item_code, item_data in items.items():
                ...
                if craft_data:
                    craft_skill = craft_data.get('skill', '').lower()
                    if craft_skill == self.skill.lower():
    ...
    def _find_consumable_items_for_skill(self, actionable_info: Dict, knowledge_base) -> None:
                ...
                if item_type in ['consumable', 'potion', 'food']:
                    ...
                    for effect in effects:
                        if isinstance(effect, dict):
                            ...
                            if self.skill.lower() in effect_name and 'xp' in effect_name:
                                actionable_info['required_items'].append({
                                    'item_code': item_code,
                                    'item_name': item_data.get('name', item_code),
                                    'action_type': 'consume',
    ...
    def __repr__(self):
        return f"FindXpSourcesAction(skill={self.skill})"