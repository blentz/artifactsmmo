...
| 20b-v2 | Per-MeansKind firing lemma: each constructor of `MeansKind` has a State predicate (the production `_fires` mirror) such that when true, the production goal's value > 0 (cite Phase-18 GoalSystem.lean). 16/17 constructors proven in `Formal/Liveness/MeansFiring.lean`; `objectiveStep` disclosed gap (Phase-18 has no StepGoal value model). 3 opaque-gated means (`lowYieldCancel`, `pursueTask`, plus `taskCancel` discharged by Phase-18's unconditional positive branch) carry their load-bearing invariants in `ProductionInvariants`; differential at 20d-v2 must exercise. | DONE_WITH_GAPS | pending |
