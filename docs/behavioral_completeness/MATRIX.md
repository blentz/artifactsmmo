...
- **Behavior coverage**: immediate-fill BOTH sides — (SELL) `DiscardOverstockGoal.relevant_actions` emits a `GeFillBuyOrderAction` for a surplus item when a standing GE buy order beats the NPC sell-back (the live `choose_venue` caller via the `liquidation_venue` adapter), alongside the NpcSell alternative; (BUY/source) `GatherMaterials.relevant_actions` emits a `GeFillSellOrderAction` for a needed input when a standing GE sell order is strictly cheaper than the NPC buy price and the acquisition decision already prefers buying (the live `choose_buy_venue` caller), alongside the NpcBuy alternative — the least-cost planner picks the venue in both cases (goals/discard_overstock.py, goals/gathering.py, actions/ge_fill.py, actions/ge_fill_sell.py, ai/liquidation_venue.py, ai/buy_source_venue.py)
- **Proof coverage**: LiquidationVenue + BuySourceVenue (DUAL pair) [dominance, totality, safety, monotonicity] — immediate-fill venue decisions over Int with Option Int. SELL side `LiquidationVenue` (sell one surplus unit into an EXISTING fillable GE buy order vs NPC sell-back, picks the HIGHER proceeds): `ge_iff_fillable_and_higher` (dominance ↔ with a true-branch witness), `venue_total`, `ge_requires_fillable_order` (anti-surrogate: GE ⇒ order isSome), `chosen_venue_maximizes` (no-value-loss; realizedProceeds coupled to actual gold), `ge_stable_under_higher_ge`/`ge_stable_under_lower_npc` (monotonicity). BUY side `BuySourceVenue` (buy one needed unit by filling an EXISTING fillable GE sell order vs NPC buy, picks the LOWER cost): the dual theorems `ge_iff_fillable_and_cheaper` (dominance), `venue_total`, `ge_requires_fillable_order` (anti-surrogate), `chosen_minimizes_cost` (no-value-loss; realizedCost ≤ npcPrice and ≤ any order), `ge_stable_under_lower_ge`/`ge_stable_under_higher_npc` (monotonicity). Both proven non-vacuously (all three branches exhibited). LIVE python cores src/artifactsmmo_cli/ai/liquidation_venue.py + ai/buy_source_venue.py driven through the oracle by formal/diff/test_liquidation_venue_diff.py + test_buy_source_venue_diff.py (PROOF_CONCEPT_INDEX)
- **Gap + policy**: CLOSED (immediate-fill BOTH sides: liquidate via buy-order fill + source via sell-order fill) — act: liquidate a surplus item by FILLING an existing GE buy order when it pays strictly more than the NPC sell-back, AND source a needed crafting input by FILLING an existing GE sell order when it costs strictly less than the NPC buy price. SELL/liquidate side (proven Option-gated `choose_venue`/`realized_proceeds`, `src/artifactsmmo_cli/ai/liquidation_venue.py` ↔ `formal/Formal/LiquidationVenue.lean`, differentially locked + mutation-killed) is LIVE-WIRED end-to-end: (1) INGESTION — `GameData._load_ge_orders` pages the BUY side of /grandexchange/orders and indexes the highest-price open buy order per item (`ge_best_buy_order(item) -> (order_id, price, quantity) | None`, ties qty-then-id; absence=`None`); (2) ADAPTER `liquidation_venue(item, qty, state, game_data)` (npc_pay=max over npcs_buying_item; ge_proceeds=ge_best_buy_order price IFF quantity>=qty else None) → proved `choose_venue`; (3) ACTION `GeFillBuyOrderAction` wraps /grandexchange/fill (sell into a standing buy order); (4) INJECTION `DiscardOverstockGoal.relevant_actions` appends `GeFillBuyOrderAction` on `Venue.GE`. Liveness PROVEN by `tests/test_ai/test_ge_liquidation_integration.py`. BUY/source side (DUAL; proven Option-gated `choose_buy_venue`/`realized_cost`, `src/artifactsmmo_cli/ai/buy_source_venue.py` ↔ `formal/Formal/BuySourceVenue.lean`, differentially locked + mutation-killed) is LIVE-WIRED end-to-end: (1) INGESTION — `_load_ge_orders` now ALSO pages the SELL side and indexes the LOWEST-price open sell order per item (`ge_best_sell_order(item) -> (order_id, price, quantity) | None`, ties qty-then-id; absence=`None`, the anti-surrogate guard; only API-sourced orders); (2) DECISION — `GatherMaterials.relevant_actions` computes `ge_price = ge_best_sell_order price IFF its quantity >= qty else None` and `choose_buy_venue(npc_price, ge_price)`; (3) ACTION `GeFillSellOrderAction` (actions/ge_fill_sell.py) wraps /my/{name}/action/grandexchange/buy (BUY from a standing sell order for immediate gold-loss/item-gain; gated on gold-cost ≥ GOLD_RESERVE + free slots; mirrors NpcBuyAction + GeFillBuyOrderAction); (4) INJECTION — when the acquisition decision already prefers BUY (`acquisition_method == Method.BUY`) AND `choose_buy_venue == BuyVenue.GE`, `GatherMaterials.relevant_actions` appends a `GeFillSellOrderAction` alongside the NpcBuy, letting the least-cost planner pick. Liveness PROVEN by `tests/test_ai/test_ge_buy_source_integration.py`: a needed input with a fillable GE sell order priced below NPC buy → emits `GeFillSellOrderAction` (choose_buy_venue==GE); no order / pricier / tie / order-qty < needed / no NPC seller → only NpcBuy (BuyVenue.NPC). grep shows the real live callers in `goals/discard_overstock.py` + `goals/gathering.py`. Full suite 100% cov, no sell/discard/gather/planner regression. (synthesis)
- **Posting policy**: IGNORE — ignore-with-reason. Posting a NEW GE order (create-buy-order / create-sell-order) is speculative liquidity-provision: a posted order MAY NEVER FILL, and the API gives no deterministic fill guarantee. The two GE journeys a leveling bot has — liquidate surplus, source bottleneck inputs — are BOTH fully covered by the immediate-fill (realizable) decisions above, so posting adds no current journey value. Modeling "posting reaches a sale/purchase" would require a fill-probability model, but the deterministic API data cannot supply one: `/grandexchange/history/{code}` records only COMPLETED sales (seller, buyer, code, quantity, price, sold_at) — it has NO record of posted-but-unfilled orders, so there is no denominator for P(fill). It yields price discovery, not a fill guarantee. A posted-price dominance proof claiming realizability would be a surrogate sham (posted price ≠ realized proceeds; the rejected-tasks-capstone failure mode in a new costume), so it is intentionally out of scope — not deferred-for-later, but deliberately not-actioned. (synthesis)
