...
- **Gap + policy**: proof landed; wiring deferred (multi-session) — the drop-driven monster SELECTION core is now PROVEN and differentially+mutation-locked: `src/artifactsmmo_cli/ai/monster_drop_selection.py` (`select_monster_for_drop` = lex-argmin over expected_kills = rate/avg_qty, tie-broken by distance then code) mirrored in `formal/Formal/MonsterDropSelection.lean` (axiom-clean {propext, Classical.choice, Quot.sound}). The DATA fix also landed (real faithfulness improvement): `_monster_drops` now restores `min_quantity` (was dropped at load) as `(code, rate, min_quantity, max_quantity)` so avg_qty=(min+max)/2 is faithful, plus a new `GameData.monsters_dropping(item)` accessor. BUT the producibility WIRING is NOT landed: `tiers/strategy._producible` + `prerequisite_graph` still treat monster-drops as non-producible leaves, and FightAction is only emitted by combat-XP goals (GrindCharacterXP/ReachUnlockLevel/unlock_bank), NOT by any `ObtainItem(drop_item)` goal — so `select_monster_for_drop` has no live caller and is INERT. An honest close still needs a new ObtainItem→FightAction goal/action path wired into the arbiter action set with `select_monster_for_drop` as the target picker (the deep `_producible`/prerequisite_graph model change). NOT CLOSED — a proven-but-inert core mislabeled CLOSED would itself be a dishonesty. Tracked as multi-session (synthesis)
...
- **Gap + policy**: MISSING (DEFERRED, multi-session) — **immediate-fill liquidation decision PROVEN; order-book ingestion + action wiring deferred**. The proven Option-gated `choose_venue`/`realized_proceeds` core is landed, differentially locked, and mutation-killed (proof landed; behavioral wiring deferred — NOT CLOSED). What remains for an honest behavioral close: (1) plumb /grandexchange/orders into game_data (a `ge_best_buy_order(item)` accessor — the GE order book is DYNAMIC live data, not part of GameData's static load-once snapshot, so it needs its own seed/refresh path, not a mechanical accessor add) and the impure `liquidation_venue(item, qty, state, game_data)` adapter; (2) a GeFillBuyOrderAction wired into a liquidation goal. Posting a NEW order (speculative; may never fill) and GE-buy-vs-craft remain explicitly OUT OF SCOPE (a posted-price proof would be a surrogate sham). Stays MISSING/DEFERRED — the decision core is proven-but-inert until (1)+(2) land (synthesis)
