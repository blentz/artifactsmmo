...
| partial | water_bow | ash_plank | 8 | 4 | 2 | PASS | `GatherMaterials(ash_plank, {ash_plank:8})` | `Recycle(water_bow×2) → Gather(ash_tree×80) → Craft(ash_plank×8)` |
| partial_protection | copper_helmet | copper_bar | 6 | 3 | 1 | PASS | `GatherMaterials(copper_bar, {copper_bar:6})` | `Recycle(copper_helmet×1) → Gather(copper_rocks×60) → Craft(copper_bar×6)` |
