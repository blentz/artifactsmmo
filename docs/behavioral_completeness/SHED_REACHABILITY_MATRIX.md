...
> GENERATED — do not hand-edit. Regenerate with `uv run python scripts/gen_shed_reachability.py`.
>
> Census drives the REAL `StrategyArbiter.select` seam over the committed bundle — the seam where the COLLECT-band shed hoist is built, so a starved rung cannot hide behind a lower seam.

5 cells; PASS 5; shed_starvation_bug 0; disposal_contradiction_bug 0
