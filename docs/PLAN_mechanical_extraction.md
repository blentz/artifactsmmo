...
> **STATUS: COMPLETE** (P1-P4, 2026-06-10 → 2026-06-11). Every pure decision core in the
> codebase (15 modules) has a mechanically extracted, kernel-bridged Lean model. See
> "Final state" below; the standing policy (new pure cores ship extracted from day one)
> lives in project memory and survives this plan.

## Final state (2026-06-11)

- **Extracted modules (15)**: ArbiterSelect, CombatPicker, CyclesForProgress,
  EquipmentScoring, EquipValue, InventoryCaps, MinGathers, NearestTile, NpcBuyCore,
  PriorityBand, RecipeClosure, ScalarCore, ShoppingList, TaskBatch, TaskReservation —
  generated by `scripts/extract_lean.py` (subset v6), sha256-stamped, byte-deterministic,
  drift gated by `formal/gate/check_extraction.sh` (gate.sh part (b''') + formal-gate CI step).
- **Bridges (Bridges.lean … Bridges7.lean)**: kernel proofs connecting every extracted def to
  its hand model (or to the spec directly where no hand model existed). No sorry/admit;
  axioms ⊆ {propext, Classical.choice, Quot.sound}. Load-bearing hand theorems transferred
  onto the extracted defs (survival floor, sticky guard-wins, clamp/strictness/tiebreaks,
  batch ≥ 1, reservation surplus-safety, closure soundness+completeness iff).
- **Four hand-model lies found by bridging** (each: Python was the spec, model corrected):
  NpcBuy Nat-truncation outside wellformedness (P1); ShoppingList constant-credit vs
  consume on DAG recipes (P2c); CyclesForProgress prevProgress kept through None readings
  (P3c); minGathers constant-credit, DAG witness 1 vs 0 (P3d). Plus latent code findings:
  order-dependent set-iteration early-return in the dominance walk (P3b), RecursionError on
  cyclic recipes (P2a/P3a, now fueled), 92-in-200k float "strict" orderings that were
  rounding noise (P4a).
- **Deferral ledger — all resolved**: shopping_list full-bridge invariant (P2a) → P2c;
  min_gathers consume gap (P2c) → P3d; recipe_closure completeness invariant (P3a) → P4c;
  equipment-scoring float cascade (P3c) → P4a/P4b.
- **Remaining trusted seams (documented, sampled, OUT of scope by design)**: the extractor
  itself (AST-level, ~480 oracle tests sample it); the single float() boundaries on the two
  learning wrappers (zero-tolerance exact cores behind them); impure wrappers/I-O shells;
  cross-core composition (covered from the other side by the arbitration/liveness models).
