...
- ~~Five children running with `--learn` share one SQLite file and need a
  `busy_timeout`.~~ **RESOLVED — no work needed.** Verified 2026-07-30: rows are
  keyed by character, WAL is enabled (`ai/learning/store.py:99`), and
  `busy_timeout` is **already 5000ms on every connection** because pysqlite's
  `connect(timeout=5.0)` default sets it — confirmed against an engine with no
  PRAGMA at all. An explicit `PRAGMA busy_timeout=5000` would be a functional
  no-op. Concurrent writers are already covered.
