...
**A DAMAGE THAT IS NOT POSITIVE FORCES NO RECOVERY, AND CONTRIBUTES NOTHING.** The guard never trips, no fight ever crosses it, and there is no chain for a recovery to end — so the first sentence's "one ends each chain" is not engaged, and neither the three-second floor nor any other part of the price applies. The per-kill contribution is zero and the loop is the bare Fight.

The condition is NOT POSITIVE rather than zero, and an earlier version of this paragraph said zero while giving the guard argument that settles the whole half-line. A negative damage is as unproducible as the handed states S-019 and S-017 legislate, and this document decides those rather than leaving them; more to the point, the guard reasoning does not distinguish them, so writing "zero" left the one case where the retained floor regime could still be claimed — its emptiness having been swept over positive damages only. An implementer could then charge three seconds to a monster that costs nothing, which changes the monster a rung NAMES and not merely its price.

Everything below is therefore scoped to positive damage, and this sentence covers the rest of the line. Without this sentence the regimes below read as exhaustive and an implementer could charge the floor's three seconds for a monster that costs nothing — which changes the monster a rung names, not merely its price, and does so exactly where armour has driven damage to nothing.
