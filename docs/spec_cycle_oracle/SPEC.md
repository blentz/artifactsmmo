...
- **The interior**, where `n · d < 1` — the chain's accumulated damage does not reach a whole bar. The recovery restores exactly what the chain spent, and **the size of the guard's band does not enter the per-kill contribution**: a longer band chains proportionally more fights and ends in a proportionally longer recovery, and the two cancel exactly. The contribution is `d` bars' worth of seconds per fight, band-free and strictly increasing in `d`. This is the regime the oracle is for, and no clause may make the band enter it.
