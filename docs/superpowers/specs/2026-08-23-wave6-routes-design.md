...
## 11. `[SHIPPED 2026-08-27]` O7 — the currency-wall census, and where it departs from §7

O7 ships as `src/artifactsmmo_cli/audit/currency_wall_census.py`, generated by
`scripts/gen_currency_wall.py --check` and wired into `formal/gate.sh` phase
c''' (now `--check x11`) and `census-gate.yml`. It was unblocked by §10's
re-measurement: the funded arm has a witness, so `REFERENCE_SET_EMPTY` no longer
fires and the residuals are falsifiable rather than trivially zero.

**Result on the committed bundle:** 264 cells over 6 currencies, 1 charged,
FUNDED 1, walled 0, `not_demanded` 251, `root_unresolved` 12, all three
residuals 0.

### How a charge is detected — and why not by walking the closure

The pricer does not report which code it walled on, and re-deriving the demand
closure inside the census would be a SECOND implementation of the cost model —
exactly what obligation **O6** forbids, and the drift it forbids it for. So the
census asks production's own pricer a differential question: price the root as
the character stands, then again with the currency granted into the bag. A price
that FALLS is a price that was paying an unobtainable charge for that currency.
One pricer, no second walk.

The known case reads `1_000_001 -> 5` on `l25_currency_leaf_unfunded`, and the
detector is pinned by tests that cross-read it against `route_price` directly
rather than by inspection.

**The test is a CROSSING, not a fall, and that is the whole precision of the
census.** Any cheaper route lowers the price. A first cut tested `price < base`,
which would have counted a currency that merely buys a material for fewer
actions than gathering it — the cell would then classify as a WALL on a currency
that walls nothing. A charge therefore requires the root to be unobtainable as
the character stands and obtainable once the currency is held. A test asserts
that every already-obtainable root in the set contributes zero charged cells.

**A single-currency probe cannot see a root walled on TWO currencies** — neither
grant alone crosses the threshold, so neither would be attributed and the gap
would go unnamed, which is the silent stall the obligation forbids. `run_census`
therefore asks one further question per scenario, granting every currency at
once; a root that crosses only then is reported as `O7_MULTI_CURRENCY_WALL`, a
fourth must-be-zero residual. It is 0 on the committed bundle, and two tests pin
that it does not fire on the ordinary cases (an obtainable root, and a root
walled on a non-currency leaf).

### Departure 1: two of §7's three wall names describe nothing

§7 proposed `WALL_GOLD`, `WALL_EVENT_ONLY` and `WALL_PASSIVE_ACCRUAL`. Measured
against the catalogue, only the middle one survives:

* **`WALL_GOLD` names nothing.** Gold is not a currency ITEM — it lives in
  `state.gold`, so a gold shortfall is charged through the buy route and never
  appears as a currency code. The six codes typed `currency` are
  `corrupted_gem`, `enchanted_coin`, `event_ticket`, `sandwhisper_coin`,
  `sonnengott_coin` and `tasks_coin`.
* **`WALL_PASSIVE_ACCRUAL` names nothing.** No currency in the catalogue accrues
  passively.

The shipped taxonomy is instead the negation of each conjunct
`obtain_sources._drop_sources` gates on, so the census and production cannot
disagree about why a currency has no route:

| Class | Condition | The currency that has this shape |
|---|---|---|
| `FUNDED` | `is_task_earnable` | `tasks_coin` (85 task rows reward it) |
| `WALL_NO_PRODUCER` | no monster's drop table lists it | `enchanted_coin`, `sonnengott_coin`, `event_ticket` |
| `WALL_EVENT_ONLY` | droppers exist, all event-gated, none with a live tile | `corrupted_gem` |
| `WALL_UNWINNABLE_DROP` | a dropper is on a live tile, none beatable at rested hp | `sandwhisper_coin` (`sandwarden`) |
| `O7_SILENT_CURRENCY_STALL` | charged, not earnable, and a BEATABLE dropper is on a live tile | none — the residual |

The winnable check precedes every wall, and that order is load-bearing: a
beatable dropper on a live tile CONTRADICTS a million-action price, and testing
the walls first would launder that contradiction into `wall_unwinnable_drop` —
the residual quietly renaming itself as an explanation. A test pins the
precedence.

**`enchanted_coin` and `sonnengott_coin` are classified on structure, not
prose.** Their flavour text says they are earned by defeating Pixie and
Sonnengott; both are raid bosses the catalogue carries with EMPTY drop tables.
Inferring the link from a description would be inventing data the API did not
give, so both take `WALL_NO_PRODUCER`, and the day those drop tables gain the
entry they reclassify by themselves.

### Departure 2: the census takes a bundle PATH, not a `GameData`

Every sibling census has the signature `run_census(game_data)`. This one is
`run_census(bundle)` and builds each scenario's declared world itself, because
a single-`GameData` signature is the defect §10 retracts: the only `tasks_coin`
vendor sits behind an achievement gate, so one default world makes the funded
arm unreachable for every cell and the sweep proves nothing while looking clean.

### The three wall arms have NO witness in the fixture set

`walled 0` is a finding, not a success, and the census prints it either way. An
unexercised classifier that no test can make fire is decorative, so each wall
arm — and both residuals — is pinned by a positive control over synthetic
evidence, in the shape of O6's `test_a_cycle_is_detected`. A separate test
asserts the wall count is currently zero, so the first fixture that exercises a
wall fails it and this section gets revisited rather than rotting.

`test_forcing_one_world_hides_the_only_funded_arm` reproduces §10's defect
directly: it prices the whole set in one default world, asserts the grid is
still full-sized, asserts NO cell is funded, and asserts the alarm fires. That is
the regression test the first run did not have.

### Two adjacent gaps closed while wiring

**1. `gen_one_cost_model.py` (O6/O8) was never in CI.** It was wired into
`formal/gate.sh` when it shipped but never added to `census-gate.yml`. Both it
and the new census are now in the workflow's path filters and step list.

**2. The `ladder/means` mutation group had been DARK since `c27599b6`** — the
increment-5.4 commit that re-denominated the bid gate. Running the gate for this
increment surfaced it, and the chain is worth recording because every link
failed quietly:

* 5.4 deleted `BID_FILL_HORIZON_SECONDS` but did not refresh the mutation anchor
  naming it, so `--check-anchors` reported 1 stale anchor;
* the anchor check runs BEFORE the mutants, so the gate stopped there and the
  group never ran;
* `formal/diff/test_ladder_fires_diff.py` still IMPORTED the deleted constant,
  so the module could not even be collected — `pytest rc=2`, which the runner
  correctly refuses to count as a kill;
* `formal/diff/` is not under the default `tests/` path, so the ordinary suite
  never collected it either.

Ten mutants were therefore unexercised for the whole of waves 5-6. All ten are
killed once the import is repaired. The `GE_BID` true-fixture then failed on its
merits: its premise was TIME (*"iron is a very-rare drop, ~1000s >> the 600s bid
horizon"*), and actions do not measure rarity that way. It is re-derived in the
new denomination — an NPC price of 5000 the character's 1000 gold cannot reach,
so self-supply means crafting 25 iron at one action each, 26 actions against a
20-cycle horizon — rather than repaired by weakening its assertion.

This is the third recorded instance of the same discipline failure: **refresh a
mutation anchor in the SAME commit as the edit that moves it.** The novel part
