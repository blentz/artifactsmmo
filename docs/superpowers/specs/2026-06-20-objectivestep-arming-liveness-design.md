...
> **RESOLUTION (2026-06-20, post-Task-3):** The OBTAINABILITY half is PROVEN 0-sorry (`canonicalPlan*` + `gear_obtainable_of_minPlanLength_le`, commit e1e5832) — this is the half the liveness chain needs (gear fieldable ⇒ winnable loadout exists). The corner-3 LOWER bound is **RETIRED, intentionally not discharged**: a production-necessity audit found it BENIGN belt-and-suspenders (sole consumer `is_plannable` is an A*-budget optimization; false-reject reroutes to incremental gather, over-admit falls through `len(plan)>0` — `strategy_driver.py:437/490`, `arbiter_select.py:84/95`). Also REFUTED: no-surplus/demand-1 does NOT crack the lower bound (the on-path case is the hard case; distribution-across-keys over the recipe-sibling foldl is the genuine open content). User signed off on retire+document+proceed. The C0 sub-tasks below for the lower-bound (`NoSurplusPlan`, `minGathers_le_gathers_nosurplus`) are CANCELLED; only the obtainability witness is delivered.
