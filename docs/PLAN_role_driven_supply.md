...
- Lor (miner) spent 95 cycles and R2D2 (logger) 97 cycles pursuing **the same** `greater_wooden_staff`. Dividing that work is what the coordination layer exists for.
  - **Annotation 2026-08-16 (final fix wave):** this example was **mis-attributed**. `greater_wooden_staff` crafts at **weaponcrafting 10**, not woodcutting — and `miner` is the role that owns weaponcrafting while `logger` does not. So the staff is Lor's *own* role's item, not R2D2's; Lor is now weaponcrafting 10 and can craft it itself. Read as "a miner and a logger duplicated the same pursuit", the duplication is real; read as "the asker needed a sibling of another role", it is wrong. **The underlying finding is unaffected:** the 10-unit `SUPPLY_DEMAND_MIN` bar rejects every quantity-1 request regardless of which role owns the item. The fixture that carried the same false fact (`crafting_skill="woodcutting"` in `tests/test_ai/test_player_coordination.py::_self_servable_gd`) was corrected in the same commit.
