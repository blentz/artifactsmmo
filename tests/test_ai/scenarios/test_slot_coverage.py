...
never endorsements. Gap index:
  ...
  GAP-1 (bag, l10_bag_pursuit): is_attainable_now's recipe walk has no
      held/banked-stock arm — 5/5 banked cowhide open nothing while cow is
      unwinnable, so the satchel chain is invisible at L10.
...
def test_l48_event_window_monsters_still_unwinnable_with_real_stats() -> None:
    ...
    derived (true) mithril-loadout stats, no monster in the L47-50 fight
    window is winnable — the event path is the only one that opens. Guards
    the honesty of the whole deliverable: if a window monster becomes
    winnable in a future bundle, the wall finding is stale and every l48
    scenario here must be revisited."""
    gd = load_bundle_game_data(BUNDLE)
    player = GamePlayer(character="l48_event_active", history=None)
    player.seed_offline(_state("l48_event_active", gd), gd)
    assert player._pick_winnable_monster() is None


# --- Deliverable 2: bag slot / satchel chain --------------------------------

def test_l10_bag_pursuit_satchel_invisible() -> None:
    """LIMITATION (GAP-1, pinned): at L10 — best L10-tier loadout, satchel's
    full monster-drop inputs BANKED (cowhide 5/5, feather 2/2), only the
    task-fundable jasper_crystal missing — the satchel is NOT attainable-now
    and bag_slot gets NO candidate: cow is unwinnable at L10 and
    is_attainable_now's recipe walk has no held/banked-stock arm, so the
    banked cowhide counts for nothing. The tree falls to the XP branch with
    NO gear fallback at all and the bot grinds slimes while a bag upgrade
    sits one task-funding loop away. Fixing the walk to credit held stock
    ...
    assert "bag_slot" not in objective.near_term_gear(state)
...
def test_l12_bag_pursuit_satchel_chain_live() -> None:
    """The minimal-delta witness that the satchel chain itself works: +2
    levels flips cow winnable (same gear), the vest is pre-equipped so it
    can't outrank the bag, and the tree targets bag_slot -> satchel as the
    SOLE candidate. The full stack routes the missing task-funded
    jasper_crystal through the C4 funding chain: ReachCurrency(tasks_coin,
    8) planning AcceptTask -> Fight -> CompleteTask. Together with the L10
    pin above this localizes GAP-1 to the attainability walk — nothing
...
def test_l35_artifact_fill_full_stack_plans_other_gear() -> None:
    """Liveness alongside the gap: the state itself is healthy — the
    planner does NOT Wait, it pursues the (non-artifact) argmax gear
    candidate through the task-funding chain."""
    report = _run("l35_artifact_fill")
    assert report.decision.chosen_root == ObtainItem(
        code="wolf_ears", quantity=1, slot="helmet_slot")
    assert repr(report.selected_goal) != "Wait"
    assert repr(report.selected_goal).startswith("ReachCurrency(tasks_coin"), (
        repr(report.selected_goal))
    assert report.plan
