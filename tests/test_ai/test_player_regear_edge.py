...
def test_player_constructs_a_regear_edge():
    p = GamePlayer(character="hero")
    assert isinstance(p._regear_edge, RegearEdge)


def test_selection_context_carries_the_narrow_level_up_flag():
    """`regear_level_up` is fed by `level_up_pending`, NOT by the raw edge.
    ...
    p = GamePlayer(character="hero")
    p._regear_edge._active = True
    p.state = make_state()
    p.game_data = _make_planner_gd()
    assert p._selection_context(combat_monster=None).regear_level_up is False

    p._regear_edge._level_up_pending = True
    assert p._selection_context(combat_monster=None).regear_level_up is True
