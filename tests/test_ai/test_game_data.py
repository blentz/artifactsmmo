...
class TestTransitiveCurrencyPassivity:
    """Passivity is a property of the CHAIN, not the last link.

    The live case (2026-07-26): lich_race_trophy costs 10 lich_race_medal, and
    each medal costs 100 event_ticket. event_ticket drops from 56/58 monsters, so
    it is passive — but lich_race_medal has ZERO droppers, so a one-level check
    called it active and armed a dedicated grind for something that really costs
    1000 tickets of ordinary combat, while craft-from-gatherables gear waited
    behind it.
    """

    @staticmethod
    def _gd(*, droppers: dict[str, list[str]], purchases: dict[str, list[tuple[str, int, str]]],
            located: bool = True, event_npcs: frozenset[str] = frozenset()):
        """A catalog big enough to clear _MIN_MONSTERS_FOR_BREADTH, wired with the
        given drop table and vendor prices."""
        gd = GameData()
        gd.monsters.levels = {f"m{i}": 1 for i in range(20)}
        gd.monsters.drops = {}
        for currency, mons in droppers.items():
            for m in mons:
                gd.monsters.drops.setdefault(m, []).append((currency, 1, 1, 1))
        for item, rows in purchases.items():
            for npc, price, currency in rows:
                gd.world.npc_stock.setdefault(npc, {})[item] = price
                gd.world.npc_buy_currency.setdefault(npc, {})[item] = currency
        gd.world.npc_tiles = {} if not located else {
            npc: (0, 0) for rows in purchases.values() for npc, _p, _c in rows}
        gd.world.npc_event_codes = {npc: "ev" for npc in event_npcs}
        return gd

    def test_direct_breadth_still_qualifies(self):
        gd = self._gd(droppers={"event_ticket": [f"m{i}" for i in range(10)]}, purchases={})
        assert gd.currency_accrues_passively("event_ticket")

    def test_a_currency_bought_with_a_passive_one_is_passive(self):
        """The lich_race_medal shape: no droppers, but bought with event_ticket."""
        gd = self._gd(
            droppers={"event_ticket": [f"m{i}" for i in range(10)]},
            purchases={"medal": [("archaeologist", 100, "event_ticket")]})
        assert gd.currency_accrues_passively("medal")

    def test_chains_of_depth_three_qualify(self):
        """Nothing about the argument stops at two links."""
        gd = self._gd(
            droppers={"event_ticket": [f"m{i}" for i in range(10)]},
            purchases={"medal": [("a", 100, "event_ticket")],
                       "trophy_token": [("a", 10, "medal")]})
        assert gd.currency_accrues_passively("trophy_token")

    def test_a_chain_that_never_reaches_a_passive_currency_stays_active(self):
        """tasks_coin's shape: earned by doing tasks, no vendor, no drops. It must
        keep arming a real grind — this is what stops the fix over-reaching."""
        gd = self._gd(droppers={"event_ticket": [f"m{i}" for i in range(10)]},
                      purchases={"jasper": [("trader", 8, "tasks_coin")]})
        assert not gd.currency_accrues_passively("tasks_coin")
        assert not gd.currency_accrues_passively("jasper")

    def test_a_currency_cycle_terminates(self):
        """A buys B and B buys A must not recurse forever."""
        gd = self._gd(droppers={}, purchases={"a": [("n", 1, "b")], "b": [("n", 1, "a")]})
        assert not gd.currency_accrues_passively("a")

    def test_an_event_vendor_does_not_confer_passivity(self):
        """A timed vendor may be closed; it cannot make anything accrue. Mirrors
        _permanent_vendor_purchases' exclusion."""
        gd = self._gd(droppers={"event_ticket": [f"m{i}" for i in range(10)]},
                      purchases={"medal": [("fair", 100, "event_ticket")]},
                      event_npcs=frozenset({"fair"}))
        assert not gd.currency_accrues_passively("medal")

    def test_an_unlocated_vendor_does_not_confer_passivity(self):
        """Same argument for a vendor with no tile: unreachable buys nothing."""
        gd = self._gd(droppers={"event_ticket": [f"m{i}" for i in range(10)]},
                      purchases={"medal": [("ghost", 100, "event_ticket")]}, located=False)
        assert not gd.currency_accrues_passively("medal")

    def test_a_small_catalog_never_qualifies_on_breadth(self):
        """The catalog floor: one dropper out of two is trivially a majority."""
        gd = GameData()
        gd.monsters.levels = {"m0": 1, "m1": 1}
        gd.monsters.drops = {"m0": [("token", 1, 1, 1)], "m1": [("token", 1, 1, 1)]}
        assert not gd.currency_accrues_passively("token")
