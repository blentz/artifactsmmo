...
class TestTreeShadowAnnotation:
    """Phase-3 Task 5: when a cycle carries a progression-tree shadow
    decision, the compact per-cycle line gains a ` tree:{==|!=}` suffix
    comparing the shadow's chosen root to the enacted `chosen_root` — the
    same repr comparison `analyze_tree_divergence` (Task 4) does over raw
    trace JSONL. No shadow this cycle (`tree_active=False`, old traces /
    unseeded cycles) means no suffix at all."""

    def test_no_suffix_when_tree_inactive(self):
        line = build_log_lines(_snap(tree_active=False))[0]
        assert "tree:" not in line

    def test_suffix_agrees_when_roots_match(self):
        line = build_log_lines(_snap(
            chosen_root="ReachCharLevel(level=6)",
            tree_active=True,
            tree_chosen_root="ReachCharLevel(level=6)",
        ))[0]
        assert "tree:==" in line

    def test_suffix_disagrees_when_roots_differ(self):
        line = build_log_lines(_snap(
            chosen_root="ReachCharLevel(level=6)",
            tree_active=True,
            tree_chosen_root="ObtainItem(code='copper_boots', quantity=1)",
        ))[0]
        assert "tree:!=" in line

    def test_suffix_agrees_when_both_roots_none(self):
        """Shadow ran and chose no root, matching a legacy cycle with no
        chosen_root either (e.g. a discretionary/interrupt cycle)."""
        line = build_log_lines(_snap(chosen_root=None, tree_active=True,
                                      tree_chosen_root=None))[0]
        assert "tree:==" in line

    def test_suffix_present_even_without_ranking(self):
        """The suffix is independent of strategy_ranking — a cycle with no
        ranking still gets exactly one line, carrying the suffix."""
        lines = build_log_lines(_snap(
            chosen_root=None, strategy_ranking=[],
            tree_active=True, tree_chosen_root=None,
        ))
        assert len(lines) == 1
        assert "tree:==" in lines[0]

    def test_suffix_on_why_line_absent_ranked_snap_default(self):
        """A ranked snapshot with no explicit tree fields (defaults) omits
        the suffix — old traces render unchanged."""
        lines = build_log_lines(_ranked_snap())
        assert "tree:" not in lines[0]
        assert "tree:" not in lines[1]
