...
class TestAPIClientWrapperCore:
    ...
    def test_create_character_validation(self, mock_create_char, mock_auth_client, mock_token_config):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('src.game_data.api_client_wrapper.CharacterSkin') as mock_skin:
                ...
                with pytest.raises(ValueError, match="Invalid skin"):
                    pytest.run(wrapper.create_character("test", "invalid_skin"))

    @patch('src.game_data.api_client_wrapper.TokenConfig')
    @patch('src.game_data.api_client_wrapper.AuthenticatedClient')
    def test_rate_limit_handling(self, mock_auth_client, mock_token_config):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('asyncio.sleep') as mock_sleep:
                pytest.run(wrapper._handle_rate_limit(mock_response))
                mock_sleep.assert_called_once_with(30)

    @patch('src.game_data.api_client_wrapper.TokenConfig')
    @patch('src.game_data.api_client_wrapper.AuthenticatedClient')
    def test_response_processing(self, mock_auth_client, mock_token_config):
        ...
        with patch.dict('sys.modules', {
            ...
            result = pytest.run(wrapper._process_response(mock_response))
            assert result == {"data": "test"}
            
            # Test error response
            mock_response.status_code = 404
            with pytest.raises(ValueError, match="Resource not found"):
                pytest.run(wrapper._process_response(mock_response))
