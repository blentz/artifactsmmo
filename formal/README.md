...
| `recipe_closure`/`_raw_units` (`ai/recipe_closure.py`) | `RecipeClosure.lean` | closure = least fixpoint (sound+complete); cyclic termination; quantity cost |
...
- **Target C (`tiers/strategy.py` helpers)**: RETIRED at THE FLIP (2026-07-07, progression tree): `_base_prior`/`_marginal`/`_value`/`_balancing`/`_learned_blend` deleted with the flat ranking. Survivors (`is_reachable`, `actionable_step`, `unmet_closure_size`, `root_cost`) were covered in Phases 2 and 9.
...
`diff/gather_xp_replay.py` REPLAYED 3231 live gather cycles on 2026-08-15: gap
10 pays 159/159, gap 11 pays **0/310**, with no outlier anywhere; the
independent craft replay over 450 valid cycles (456 raw, 6 skipped) splits the
same way. Hence `GREY_SKILL_GAP = 11`. Past tense on purpose: the
`play-trace-*.jsonl` corpus behind both runs was deleted the same day, and both
harnesses now read the learning store and report ZERO usable observations until
`cycles.skill_levels_json` / `craft_yield.skill_level` accumulate. The number
stands on that one-time observation plus the mutation group; re-running either
script today re-derives nothing.
...
combat's boundary too: `diff/xp_formula_replay.py` over 10 883 ok-fights finds
