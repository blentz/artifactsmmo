...
recipe closure (target + every transitive intermediate + raw resource):
    ...
    obtainProgress r fuel owned nodes = Σ_{i ∈ nodes} owned i * rawUnits r fuel i
...
def obtainProgress (r : Recipe) (fuel : Nat) (owned : Nat → Nat) (nodes : List Nat) : Nat :=
  (nodes.map (fun i => owned i * rawUnits r fuel i)).sum

/-- On a list not containing `g`, the gather update is invisible — the mapped sum is
unchanged. -/
private theorem sum_gather_no_g (r : Recipe) (fuel : Nat) (owned : Nat → Nat)
    (nodes : List Nat) (g k : Nat) (h : g ∉ nodes) :
    (nodes.map (fun i => (owned i + (if i = g then k else 0)) * rawUnits r fuel i)).sum
      = (nodes.map (fun i => owned i * rawUnits r fuel i)).sum := by
...
witness by exactly `k * rawUnits r fuel g`. The exact delta law the gather/strict-mono
corollaries rest on. -/
theorem obtainProgress_gather (r : Recipe) (fuel : Nat) (owned : Nat → Nat)
    (nodes : List Nat) (g k : Nat) (hg : g ∈ nodes) (hnd : nodes.Nodup) :
    obtainProgress r fuel (fun j => owned j + (if j = g then k else 0)) nodes
      = obtainProgress r fuel owned nodes + k * rawUnits r fuel g := by
  ...
  | cons hd tl ih =>
    ...
    · -- g is the head; the tail never mentions g (Nodup), so its sum is unchanged.
      subst hgeq
      rw [sum_gather_no_g r fuel owned tl g k hhd, if_pos rfl]
      ring
...
theorem obtainProgress_mono (r : Recipe) (fuel : Nat) (owned owned' : Nat → Nat)
    (nodes : List Nat) (h : ∀ i, owned i ≤ owned' i) :
    obtainProgress r fuel owned nodes ≤ obtainProgress r fuel owned' nodes := by
  unfold obtainProgress
  induction nodes with
  | nil => simp
  | cons hd tl ih =>
...
theorem obtainProgress_gather_strict (r : Recipe) (fuel : Nat) (owned : Nat → Nat)
    (nodes : List Nat) (g : Nat) (hg : g ∈ nodes) (hnd : nodes.Nodup)
    (hpos : 0 < rawUnits r fuel g) :
    obtainProgress r fuel owned nodes
      < obtainProgress r fuel (fun j => owned j + (if j = g then 1 else 0)) nodes := by
  rw [obtainProgress_gather r fuel owned nodes g 1 hg hnd]
  omega

/-- Consuming `k` units of a node `c` (present once in `nodes`, with `k ≤ owned c`) lowers
the witness by exactly `k * rawUnits r fuel c`. Dual of `obtainProgress_gather`; the craft
theorem pairs a production (+output) with this consumption (−inputs). -/
theorem obtainProgress_consume (r : Recipe) (fuel : Nat) (owned : Nat → Nat)
    (nodes : List Nat) (c k : Nat) (hc : c ∈ nodes) (hnd : nodes.Nodup) (hk : k ≤ owned c) :
    obtainProgress r fuel (fun j => owned j - (if j = c then k else 0)) nodes
      + k * rawUnits r fuel c = obtainProgress r fuel owned nodes := by
  unfold obtainProgress
  induction nodes with
  ...
  | cons hd tl ih =>
    ...
    · subst hceq
      have htl_eq : (tl.map (fun i => (owned i - (if i = c then k else 0)) * rawUnits r fuel i)).sum
          = (tl.map (fun i => owned i * rawUnits r fuel i)).sum := by
      ...
      have : k * rawUnits r fuel c ≤ owned c * rawUnits r fuel c :=
...
theorem obtainProgress_craft_invariant (r : Recipe) (fuel : Nat) (owned : Nat → Nat)
    (nodes : List Nat) (o c qty : Nat) (ho : o ∈ nodes) (hcm : c ∈ nodes) (hoc : o ≠ c)
    (hnd : nodes.Nodup) (hq : qty ≤ owned c)
    (hcons : rawUnits r fuel o = qty * rawUnits r fuel c) :
    obtainProgress r fuel
        (fun j => (owned j + (if j = o then 1 else 0)) - (if j = c then qty else 0)) nodes
      = obtainProgress r fuel owned nodes := by
  ...
  have key := obtainProgress_consume r fuel (fun j => owned j + (if j = o then 1 else 0))
    nodes c qty hcm hnd (by simp [hoc.symm]; exact hq)
  have prod := obtainProgress_gather r fuel owned nodes o 1 ho hnd
