...
theorem armor_score_bridge (enc : Int → String)
    ...
    (hpBonus wisdom prospecting inventorySpace haste : Int)
    (hflat : item.flatUtil = hpBonus + wisdom + prospecting + inventorySpace + haste) :
    Extracted.EquipmentScoring.armor_score_pure
        (Formal.EquipmentScoring.elements.map enc)
        (encElem enc item.resistance) (encElem enc monsterAtk)
        hpBonus wisdom prospecting inventorySpace haste
...
theorem equip_value_bridge (attack resistance hpRestore hpBonus dmg crit wisdom prospecting inventorySpace haste : Int)
    (subtype : String) :
    Extracted.EquipValue.equip_value_pure
        attack resistance hpRestore hpBonus dmg crit wisdom prospecting inventorySpace haste subtype
      = Formal.EquipValueAugmented.equipValue
        ⟨attack, resistance, hpRestore, hpBonus, dmg, crit, wisdom, prospecting, inventorySpace, haste⟩
        (subtype == "tool") := by
...
theorem equip_value_strict_extracted
    ...
    Extracted.EquipValue.equip_value_pure
        s.attack s.resistance s.hpRestore s.hpBonus s.dmg s.crit s.wisdom s.prospecting s.inventorySpace s.haste subS
      < Extracted.EquipValue.equip_value_pure
        t.attack t.resistance t.hpRestore t.hpBonus t.dmg t.crit t.wisdom t.prospecting t.inventorySpace t.haste subT := by
...
theorem equip_value_tiebreak_extracted
    ...
    (hTie : Formal.EquipValueAugmented.rawSum s
      = Formal.EquipValueAugmented.rawSum t) :
    Extracted.EquipValue.equip_value_pure
        s.attack s.resistance s.hpRestore s.hpBonus s.dmg s.crit s.wisdom s.prospecting s.inventorySpace s.haste "tool"
      < Extracted.EquipValue.equip_value_pure
        t.attack t.resistance t.hpRestore t.hpBonus t.dmg t.crit t.wisdom t.prospecting t.inventorySpace t.haste subT := by
  rw [equip_value_bridge, equip_value_bridge]
