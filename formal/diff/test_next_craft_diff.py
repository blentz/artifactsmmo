...
def _call_lean(
    ...
    result = run_oracle_structured(
        "next_craft",
        [[recipes_json, owned_json, target, qty, fuel]],
    )[0]
...
def test_next_craft_agrees(recipe_seed: int, owned_seed: int, qty: int) -> None:
    """Python core ≡ Lean model on random acyclic DAGs."""
    ...
    target = "item0"
    py = next_craft_target_pure(recipes, owned, target, qty)
    lean = _call_lean(recipes, owned, target, qty)
    _assert_agree(py, lean, (recipe_seed, owned_seed, qty))
...
def test_copper_ring_gather_ore() -> None:
    ...
    owned: dict[str, int] = {}
    qty = 3
    py = next_craft_target_pure(_COPPER_RECIPES, owned, "copper_ring", qty)
    lean = _call_lean(_COPPER_RECIPES, owned, "copper_ring", qty)
